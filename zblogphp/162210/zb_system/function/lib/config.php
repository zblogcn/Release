...
class Config implements Iterator
{
    ...
    public function Save()
    {
        ...
        if ($this->db->type == 'sqlite') {
            ...
            $old = @$this->db->Query("SELECT column_name FROM information_schema.columns WHERE table_schema='$dbname' AND table_name = '$this->table' AND column_name = 'conf_Key'");
        ...
        $old3 = $this->db->Query($this->db->sql->Select($this->table, '*', array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', ''))));
        if (count($old3) > 0) { //如果存在老数据，先删除老的
            $del = array();
            $mod = array();
            $add = $this->kvdata;

            $sql = $this->db->sql->Delete($this->table, array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', '')));
            $this->db->Delete($sql);
        }
        if (($add + $del + $mod) == array()) {
            return true;
        ...
        foreach ($add as $key2 => $value2) {
            $kv2 = array($this->datainfo['Name'][0] => $name, 'conf_Key' => $key2, $this->datainfo['Value'][0] => $this->SerializeSingle($value2));
            $sql2 = $this->db->sql->Insert($this->table, $kv2);
            $old4 = $this->db->Query($this->db->sql->Select($this->table, '*', array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', $key2))));
            if (count($old4) == 0) {
                $this->db->Insert($sql2);
            } else {
                $key3 = $key2;
                ...
                $sql3 = $this->db->sql->Update($this->table, $kv3, array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', $key3)));
                $this->db->Update($sql3);
            }
        }
        //mod
        foreach ($mod as $key4 => $value4) {
            $kv3 = array($this->datainfo['Name'][0] => $name, 'conf_Key' => $key4, $this->datainfo['Value'][0] => $this->SerializeSingle($value4));
            $sql4 = $this->db->sql->Insert($this->table, $kv3);
            $old5 = $this->db->Query($this->db->sql->Select($this->table, '*', array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', $key4))));
            if (count($old5) == 0) {
                $this->db->Insert($sql4);
            } else {
                $key5 = $key4;
                ...
                $sql5 = $this->db->sql->Update($this->table, $kv3, array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', $key5)));
                $this->db->Update($sql5);
            }
        }
        //del
        foreach ($del as $key6 => $value6) {
            $sql6 = $this->db->sql->Delete($this->table, array(array('=', $this->datainfo['Name'][0], $name), array('=', 'conf_Key', $key6)));
            $this->db->Delete($sql6);
        }
